...
import static com.hujiang.restvolley2.image.ImageProcessor.BYTE_IN_PIX;
...
public class RVImageRequest extends Request<Bitmap> {
    ...
    private Bitmap doParseStreamSafe(InputStream bitmapStream, long contentLength, String contentType) {
        ...
        if (!"image/webp".equals(lowerContentType) && contentLength <= RVImageCache.MAX_BITMAP_CACHE_SIZE) {
            decodeOptions.inJustDecodeBounds = false;
            decodeOptions.inPreferredConfig = mDecodeConfig;
        ...
        String diskCachePath = RVImageLoader.instance(mContext).getDiskCachePath();
        ...
        if (mMaxWidth == 0 && mMaxHeight == 0) {
            ...
            while (desired_size > RVImageCache.MAX_BITMAP_CACHE_SIZE) {
