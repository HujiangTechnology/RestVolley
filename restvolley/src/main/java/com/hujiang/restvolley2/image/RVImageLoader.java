 ...
 * @since 2015-11-24
 */
public class RVImageLoader {

    private static final String IMAGE_REQUEST_ENGINE_TAG = "image";
    private static RVImageLoader sInstance;

    private RequestEngine mRequestEngine;
    private ImageLoaderCompat mImageLoader;
    private RVImageCache mRVImageCache;

    private Context mContext;

    private RVImageLoader(Context context) {
        mContext = context.getApplicationContext();

        mRequestEngine = RestVolley.newRequestEngine(context, IMAGE_REQUEST_ENGINE_TAG, true);

        mRVImageCache = new RVImageCache(context);
        mImageLoader = new ImageLoaderCompat(mContext, mRequestEngine.requestQueue, mRVImageCache);
    }

    /**
     * singleton of {@link RVImageLoader}.
     * @param context {@link Context}
     * @return {@link RVImageLoader}
     */
    public static RVImageLoader instance(Context context) {
        if (sInstance == null) {
            synchronized (RVImageLoader.class) {
                if (sInstance == null) {
                    sInstance = new RVImageLoader(context);
                }
            }
        }
    ...
    public void setConfig(ImageLoaderGlobalConfig config) {
        if (config != null) {
            ...
            mRVImageCache = new RVImageCache(mContext, config.memCacheSize, config.diskCacheSize, config.diskCacheDir);
            mImageLoader = new ImageLoaderCompat(mContext, mRequestEngine.requestQueue, mRVImageCache);
        }
    }
    ...
    public String getDiskCachePath(String uri, int maxWidth, int maxHeight, ImageView.ScaleType scaleType) {
        return mRVImageCache.getDiskCachePath(ImageLoaderCompat.generateCacheKey(uri, maxWidth, maxHeight, scaleType));
    }
    ...
    public File getDiskCacheDir() {
        return mRVImageCache.getDiskCacheDir();
    }
    ...
    public String getDiskCachePath() {
        return mRVImageCache.getDiskCachePath();
    }
}