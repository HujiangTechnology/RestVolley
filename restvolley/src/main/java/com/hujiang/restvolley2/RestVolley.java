...
public class RestVolley extends Volley {
    ...
    public static RequestEngine newRequestEngine(Context context, String engineTag, boolean isStreamBased) {
        ...
        if (requestEngine == null) {
            ...
            okHttpBuilder.connectTimeout(DEFAULT_HTTP_TIMEOUT, TimeUnit.MILLISECONDS)
                    .readTimeout(DEFAULT_HTTP_TIMEOUT, TimeUnit.MILLISECONDS)
                    .writeTimeout(DEFAULT_HTTP_TIMEOUT, TimeUnit.MILLISECONDS)
                    .hostnameVerifier(CertificateUtils.ALLOW_ALL_HOSTNAME_VERIFIER)
                    .sslSocketFactory(CertificateUtils.getDefaultSSLSocketFactory(), CertificateUtils.getDefaultTrustManager());
    ...
    public static RequestQueue newRequestQueue(Context context, HttpStack stack, int maxDiskCacheBytes, int threadPoolSize, boolean isStreamBasedResponse) {
        ...
        RequestQueue queue;
        RVResponseDelivery delivery = new RVResponseDelivery(new Handler(Looper.getMainLooper()));
