...
package com.hujiang.restvolley2.download;
...
public class RVDownload {
     ...
     */
    public RVDownload(Context context) {
     ...
     */
    public RVDownload(RequestEngine requestEngine) {
        mRequestBuilder = new Request.Builder();
        mRequestBuilder.get();
        mOkHttpClient = requestEngine.okHttpClient;
    }

    public RVDownload setSSLSocketFactory(SSLSocketFactory sslSocketFactory) {
        mSSLSocketFactory = sslSocketFactory;

        return this;
    }

    public RVDownload setHostnameVerifier(HostnameVerifier hostnameVerifier) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload url(String url) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload addHeader(String key, String value) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload addHeader(Headers headers) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload tag(Object tag) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload setProxy(Proxy proxy) {
        mProxy = proxy;
        return this;
    }
     ...
     * @return {@link RVDownload}
     */
    public RVDownload setProxy(String host, int port) {
    ...
    public RVDownload setUserAgent(String userAgent) {
        if (!TextUtils.isEmpty(userAgent)) {
            addHeader(RestVolley.HEADER_USER_AGENT, userAgent);
        }
     ...
     * @return {@link RVDownload}
     */
    public RVDownload setConnectTimeout(long timeout) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload setReadTimeout(long timeout) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload setWriteTimeout(long timeout) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload setTimeout(long timeout) {
     ...
     * @return {@link RVDownload}
     */
    public RVDownload setIsAppend(boolean isAppend) {
        mIsAppend = isAppend;
    ...
    public void download(final String localPath, Callback callback) {
        bindOkHttpClient().newCall(mRequestBuilder.get().build()).enqueue(callback);
    }
    ...
    public void download(final String localPath, final OnDownloadListener listener) {
        ...
        bindOkHttpClient().newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                notifyDownloadError(url, e, -1, null, listener);
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                try {
                    ...
                    if (response.isSuccessful() && writeStream2File(response, localFile, mIsAppend, listener)) {
                        notifyDownloadSuccess(url, new File(localPath), response.code(), response.headers(), listener);
                    } else {
                        notifyDownloadError(url, new Exception("download does not complete:" + localFile.getAbsolutePath())
                                , response.code(), response.headers(), listener);
                    }
                } catch (Exception e) {
                    notifyDownloadError(url, e, response.code(), response.headers(), listener);
                }
            }
        });
    }
    ...
    public static void download(Context context, String url, final String localPath, final OnDownloadListener listener) {
        new RVDownload(context).url(url).download(localPath, listener);
    }
    ...
    public DownloadResponse syncDownload(String localPath) {
        ...
        try {
            Response result = bindOkHttpClient().newCall(mRequestBuilder.build()).execute();
    ...
    public static DownloadResponse syncDownload(Context context, String url, String localPath) {
        return new RVDownload(context).url(url).syncDownload(localPath);
    }
        ...
        if (inputStream != null) {
            ...
            try {
                ...
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    notifyDownloadProgress(response.request().url().toString(), count, totalBytes, localFile, response.code()
